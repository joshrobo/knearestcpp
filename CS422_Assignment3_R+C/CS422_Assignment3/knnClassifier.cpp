...
namespace knn422 {
		...
		if (testFile.is_open()) {
			...
			while (getline(testFile, line)) {
				...
				while (getline(ss, token, ',')) {
					...
					token = scaleBasedOn(token, attribute_index, dsinfo.scale_setting);
	...
	string scaleBasedOn(string token, int att_index, map<int, map<string, string>> scale_set) {
		//based on attribute index
		//based on token
		//change token according to user setting
		if (scale_set.empty()) { return token; }
		else {
			return scale_set.at(att_index).at(token);
		}
	}
